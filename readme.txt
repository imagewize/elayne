...
* FIXED: package.json failing the Theme Check GitHub Action — its keywords array had lowercase "wordpress", tripping the required WordPress_Spelling_Check rule, corrected to "WordPress". Also caught while in there: version was still 4.9.0 and license still GPL-2.0-or-later (same stale GPL v2 issue as composer.json, above); both corrected, and description's pattern count refreshed to match.
...
* FIXED: Wide sections inside the page patterns rendered full-viewport instead of at the theme's wide width, and picked up an unintended gap between sections, because of the `is-layout-flow` wrapper group. The wrapper has been removed.
* FIXED: `package.json` version was out of sync with `style.css` and `readme.txt`.
