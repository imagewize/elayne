...
| **Composite page patterns** | Compose `page-*.php` patterns from `<!-- wp:pattern {"slug":"elayne/..."} /-->` refs (Ollie convention) — NEVER PHP `include`. Slugs survive file renames and resolve through the registry, so child themes can override a section; `include get_template_directory()` hardcodes a path and is parent-theme-only. Add NO wrapper group: an `alignfull` group with `layout:default` renders `is-layout-flow`, which has no `.alignwide` rule (sections lose their wide-size cap) and adds `margin-block-start: 1.2rem` between every section. Sentinel will always report `content_mismatch` + `content_injected` for these — the editor substitutes each ref with its blocks on insert. Known limitation, not a pattern bug. |
