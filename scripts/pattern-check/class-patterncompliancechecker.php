...
class PatternComplianceChecker {
	...
	/**
	 * Check all patterns in the directory
	 *
	 * @return void
	 */
	public function check_all_patterns() {
		$patterns = glob( $this->pattern_dir . '/*.php' );

		foreach ( $patterns as $pattern_file ) {
			$pattern_name = basename( $pattern_file );
			// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- Reading local pattern files, not remote URLs.
	...
	public function print_results() {
		...
		foreach ( $results['passed'] as $pattern ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CLI output, pattern is a local filename.
			echo '  - ' . $pattern . "\n";
		}
		...
		if ( ! empty( $results['issues'] ) ) {
			...
			foreach ( $results['issues'] as $pattern => $issues ) {
				...
				foreach ( $issues as $issue ) {
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CLI output, issue is a static string.
					echo '    - ' . $issue . "\n";
				}
			}
		} else {
...
if ( 'cli' === php_sapi_name() ) {
	...
	if ( ! is_dir( $pattern_dir ) ) {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CLI output, pattern_dir is a local path.
